...
class JKBt(BtBms):
    ...
    async def fetch_cell_resistances(self):
        if self.num_cells is None:
            raise Exception("num_cells not set")
        buf, t_buf = self._resp_table[0x02]
        fw_offset = 32 if self.is_new_11fw_32s else 0
        offset = 80
        resistances = [int.from_bytes(buf[(offset + i * 2):(offset + i * 2 + 2)], byteorder="little") for i in range(self.num_cells)]
        return resistances
    ...
    async def set_setting(self, key: str, value: float):
        import struct
        SETTINGS_MAP = {
            "smart_sleep_voltage":                           (0x01, 1000, False),
            "cell_voltage_undervoltage_protection":          (0x02, 1000, False),
            "cell_voltage_undervoltage_recovery":            (0x03, 1000, False),
            "cell_voltage_overvoltage_protection":           (0x04, 1000, False),
            "cell_voltage_overvoltage_recovery":             (0x05, 1000, False),
            "balance_trigger_voltage":                       (0x06, 1000, False),
            "cell_soc100_voltage":                           (0x07, 1000, False),
            "cell_soc0_voltage":                             (0x08, 1000, False),
            "cell_request_charge_voltage":                   (0x09, 1000, False),
            "cell_request_float_voltage":                    (0x0a, 1000, False),
            "power_off_voltage":                             (0x0b, 1000, False),
            "max_charge_current":                            (0x0c, 1000, False),
            "charge_overcurrent_protection_delay":           (0x0d, 1,    False),
            "charge_overcurrent_protection_recovery_time":   (0x0e, 1,    False),
            "max_discharge_current":                         (0x0f, 1000, False),
            "discharge_overcurrent_protection_delay":        (0x10, 1,    False),
            "discharge_overcurrent_protection_recovery_time":(0x11, 1,    False),
            "short_circuit_protection_recovery_time":        (0x12, 1,    False),
            "max_balance_current":                           (0x13, 1000, False),
            "charge_overtemperature_protection":             (0x14, 10,   False),
            "charge_overtemperature_protection_recovery":    (0x15, 10,   False),
            "discharge_overtemperature_protection":          (0x16, 10,   False),
            "discharge_overtemperature_protection_recovery": (0x17, 10,   False),
            "charge_undertemperature_protection":            (0x18, 10,   True),
            "charge_undertemperature_protection_recovery":   (0x19, 10,   True),
            "power_tube_overtemperature_protection":         (0x1a, 10,   False),
            "power_tube_overtemperature_protection_recovery":(0x1b, 10,   False),
            "cell_count":                                    (0x1c, 1,    False),
            "total_battery_capacity_ah":                     (0x20, 1000, False),
            "short_circuit_protection_delay":                (0x21, 1,    False),
            "balance_starting_voltage":                      (0x22, 1000, False),
            "connexion_wire_resistance_1":                   (0x27, 1000, False),
            "device_address":                                (0x23, 1,    False),
        }
        if key not in SETTINGS_MAP:
            self.logger.warning("set_setting: unknown key %s", key)
            return
        address, scale, signed = SETTINGS_MAP[key]
        raw = int(round(value * scale))
        if signed:
            payload = list(struct.pack('<i', raw))
        else:
            payload = list(struct.pack('<I', raw))
        self.logger.info("set_setting %s=%s addr=0x%02x raw=%d", key, value, address, raw)
        await self._write(address, payload)
        await asyncio.sleep(0.2)
        self._resp_table.pop(0x01, None)
