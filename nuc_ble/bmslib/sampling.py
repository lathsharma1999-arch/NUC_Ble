...
from bmslib.mqtt_util import publish_sample, publish_cell_voltages, publish_cell_resistances, publish_temperatures, publish_hass_discovery, subscribe_settings, publish_settings, \
    ...
    async def _sample_inner(self):
        ...
        async with bms:
            ...
            if self.num_samples == 0 and sample.switches and mqtt_client:
                ...
                subscribe_settings(mqtt_client, device_topic=self.mqtt_topic_prefix, bms=bms)
            ...
            if self.period_discov or self.period_pub or \
                ...
                if isinstance(bms, JKBt):
                    try:
                        publish_settings(mqtt_client, device_topic=self.mqtt_topic_prefix, bms=bms)
                    except Exception as e:
                        logger.warning("publish_settings failed: %s", e)
                ...
                try:
                    ...
                    logger.warning('cached_fetch_voltages failed: %s', e)
                try:
                    from bmslib.models.jikong import JKBt
                    if isinstance(bms, JKBt):
                        resistances = await bms.fetch_cell_resistances()
                        publish_cell_resistances(mqtt_client, device_topic=self.mqtt_topic_prefix, resistances=resistances)
                except Exception as e:
