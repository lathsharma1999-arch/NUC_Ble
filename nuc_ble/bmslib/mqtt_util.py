...
def publish_cell_resistances(client, device_topic, resistances):
    if not resistances:
        return
    for i, r in enumerate(resistances):
        topic = f"{device_topic}/cell_resistances/{i + 1}"
        mqtt_single_out(client, topic, round(r / 1000, 4))
...
def subscribe_settings(mqtt_client, device_topic, bms):
    from bmslib.models.jikong import JKBt
    if not isinstance(bms, JKBt):
        return

    SETTINGS_KEYS = [
        "smart_sleep_voltage", "cell_voltage_undervoltage_protection",
        "cell_voltage_undervoltage_recovery", "cell_voltage_overvoltage_protection",
        "cell_voltage_overvoltage_recovery", "balance_trigger_voltage",
        "cell_soc100_voltage", "cell_soc0_voltage", "cell_request_charge_voltage",
        "cell_request_float_voltage", "power_off_voltage", "max_charge_current",
        "charge_overcurrent_protection_delay", "charge_overcurrent_protection_recovery_time",
        "max_discharge_current", "discharge_overcurrent_protection_delay",
        "discharge_overcurrent_protection_recovery_time", "short_circuit_protection_recovery_time",
        "max_balance_current", "charge_overtemperature_protection",
        "charge_overtemperature_protection_recovery", "discharge_overtemperature_protection",
        "discharge_overtemperature_protection_recovery", "charge_undertemperature_protection",
        "charge_undertemperature_protection_recovery", "power_tube_overtemperature_protection",
        "power_tube_overtemperature_protection_recovery", "cell_count",
        "total_battery_capacity_ah", "short_circuit_protection_delay",
        "balance_starting_voltage", "connexion_wire_resistance_1", "device_address",
    ]

    async def set_setting(key, payload):
        try:
            value = float(payload)
            await bms.set_setting(key, value)
            topic = f"{device_topic}/control/{key}/state"
            mqtt_single_out(mqtt_client, topic, str(value), retain=True)
            logger.info("set_setting %s = %s OK", key, value)
        except Exception as e:
            logger.error("set_setting %s failed: %s", key, e)

    for key in SETTINGS_KEYS:
        topic = f"{device_topic}/control/{key}/set"
        logger.debug("subscribe setting %s", topic)
        mqtt_client.subscribe(topic, qos=0)
        _switch_callbacks[topic] = lambda msg, k=key: set_setting(k, msg)


def publish_settings(mqtt_client, device_topic, bms):
    import struct
    try:
        buf, _ = bms._resp_table[0x01]
    except Exception as e:
        logger.warning("publish_settings: no 0x01 in resp_table: %s", e)
        return

    def u32(i): return int.from_bytes(buf[i:i+4], 'little', signed=False)
    def s32(i): return int.from_bytes(buf[i:i+4], 'little', signed=True)

    settings = {
        "smart_sleep_voltage":                           round(u32(6)   * 0.001, 3),
        "cell_voltage_undervoltage_protection":          round(u32(10)  * 0.001, 3),
        "cell_voltage_undervoltage_recovery":            round(u32(14)  * 0.001, 3),
        "cell_voltage_overvoltage_protection":           round(u32(18)  * 0.001, 3),
        "cell_voltage_overvoltage_recovery":             round(u32(22)  * 0.001, 3),
        "balance_trigger_voltage":                       round(u32(26)  * 0.001, 3),
        "cell_soc100_voltage":                           round(u32(30)  * 0.001, 3),
        "cell_soc0_voltage":                             round(u32(34)  * 0.001, 3),
        "cell_request_charge_voltage":                   round(u32(38)  * 0.001, 3),
        "cell_request_float_voltage":                    round(u32(42)  * 0.001, 3),
        "power_off_voltage":                             round(u32(46)  * 0.001, 3),
        "max_charge_current":                            round(u32(50)  * 0.001, 3),
        "charge_overcurrent_protection_delay":           u32(54),
        "charge_overcurrent_protection_recovery_time":   u32(58),
        "max_discharge_current":                         round(u32(62)  * 0.001, 3),
        "discharge_overcurrent_protection_delay":        u32(66),
        "discharge_overcurrent_protection_recovery_time":u32(70),
        "short_circuit_protection_recovery_time":        u32(74),
        "max_balance_current":                           round(u32(78)  * 0.001, 3),
        "charge_overtemperature_protection":             round(u32(82)  * 0.1,   1),
        "charge_overtemperature_protection_recovery":    round(u32(86)  * 0.1,   1),
        "discharge_overtemperature_protection":          round(u32(90)  * 0.1,   1),
        "discharge_overtemperature_protection_recovery": round(u32(94)  * 0.1,   1),
        "charge_undertemperature_protection":            round(s32(98)  * 0.1,   1),
        "charge_undertemperature_protection_recovery":   round(s32(102) * 0.1,   1),
        "power_tube_overtemperature_protection":         round(s32(106) * 0.1,   1),
        "power_tube_overtemperature_protection_recovery":round(s32(110) * 0.1,   1),
        "cell_count":                                    buf[114],
        "total_battery_capacity_ah":                     round(u32(130) * 0.001, 3),
        "short_circuit_protection_delay":                u32(134),
        "balance_starting_voltage":                      round(u32(138) * 0.001, 3),
    }
